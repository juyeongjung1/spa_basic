...
演習では、次の3フォルダを使用します。受講者は `sample` を参照しながら `exercise_question` の問題を解き、動作確認後に `exercise_answer` と比較してください。

```text
sample/             メインテキストで使用する参照サンプル
exercise_question/  受講者が編集する問題ファイル
exercise_answer/    演習の完成解答
```

`exercise_question` と `exercise_answer` は同じ相対パス・ファイル名で対応させます。例えば、問題が `exercise_question\Ex1-4\innerHTML.html` の場合、完成解答は `exercise_answer\Ex1-4\innerHTML.html` です。
...
参照テキスト: 第3章 章のまとめ
参照ファイル: `sample\Ex3-3`～`sample\Ex3-6` の完成コード
...
非同期通信では、通信完了を待つ間もブラウザの他の処理やユーザー操作を継続できます。本演習では、通信完了後の処理を `then` で記述します。
