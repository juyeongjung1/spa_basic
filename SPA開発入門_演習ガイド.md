...
1. `showEmployeeList(employees)` の中にある更新ボタンの `onclick` を確認し、対象社員の社員番号を `setUpdateForm` 関数へ渡してください。社員一覧には、社員番号の左に顔写真列を表示します。
2. `setUpdateForm(id)` の中で、GET `http://localhost:3005/api/employees/${id}` を呼び出し、対象社員の詳細情報を取得してください。
3. 対象社員のパスワード以外の現在値をフォームへ表示してください。
4. 詳細APIで取得した `image_path` を画像パス入力欄へ表示し、`image_path` がある場合だけ社員画像を表示してください。更新フォームには既存値が入るため、placeholderは不要です。
5. 更新時はパスワードを含む5項目を取得してください。
6. PUT `http://localhost:3005/api/employees/${id}` を呼び出してください。
7. 更新成功後に社員一覧を再取得してください。
...
7. `location_name` と `image_path` は、フロントエンドから送られるキー名と一致させて取得してください。
8. 次のSQLをプレースホルダー付きで実行してください。
...
9. 更新後に社員一覧を取得し、最新の一覧をJSON形式で返してください。
...
`locationName` には `req.body.location_name`、`imagePath` には `req.body.image_path` を代入します。画面側の入力欄IDとは名前が少し違うため、ここは特に注意してください。
