...
演習4.3は、URLのパス（`/employees` など）を読み取って表示画面を切り替えるSPAです。`index.html` をVS CodeのLive Serverで直接開くと、URLが `/exercise_question/Ex4-3/index.html` のようになり、ルート判定を正しく確認できません。

動作確認では、次の手順でブラウザのアドレス欄へURLを直接入力してください。

1. `exercise_question\Ex4-3\api` で `npm run dev` または `npm start` を実行し、APIサーバーを起動してください。
2. VS Codeで `exercise_question\Ex4-3` フォルダを開き、そのフォルダをLive Serverのルートとして起動してください。
3. Live Serverのポート番号を確認してください。通常は `http://127.0.0.1:5500` です。ポート番号が異なる場合は、以降のURLを読み替えてください。
4. ブラウザのアドレス欄へ、確認したいURLを直接入力してください。

例:

- `http://127.0.0.1:5500/`
- `http://127.0.0.1:5500/employees`
- `http://127.0.0.1:5500/employees/new`
- `http://127.0.0.1:5500/employees/1001`

完成解答を確認する場合は、同じ手順で `exercise_sample\Ex4-3` フォルダをLive Serverのルートとして起動してください。
...
- ブラウザのアドレス欄に `http://127.0.0.1:5500/` を直接入力し、ホーム画面が表示されること
- ブラウザのアドレス欄に `http://127.0.0.1:5500/employees` を直接入力し、社員一覧が表示されること
- ブラウザのアドレス欄に `http://127.0.0.1:5500/employees/new` を直接入力し、新規社員登録画面が表示されること
- ブラウザのアドレス欄に `http://127.0.0.1:5500/employees/1001` を直接入力し、社員詳細が表示されること
...
- ブラウザのアドレス欄に `http://127.0.0.1:5500/invalid_path_test` を直接入力した際、上の画像のように「ページが見つかりません」と入力したパスが表示されること
...
![演習4.3.3 動作確認](./images/4-3-3_about.png)

- メニューの「このシステムについて」またはブラウザのアドレス欄に `http://127.0.0.1:5500/about` を直接入力した際、上の画像のようにシステム概要および使用技術が表示されること

## 演習4.4 【参考】コンポーネント化

### 演習4.4の動作確認方法

演習4.4も演習4.3と同じく、URLのパスを読み取って画面を切り替えます。`index.html` を直接開くのではなく、次の手順でURLを直接入力して確認してください。

1. `exercise_question\Ex4-4\api` で `npm run dev` または `npm start` を実行し、APIサーバーを起動してください。
2. VS Codeで `exercise_question\Ex4-4` フォルダを開き、そのフォルダをLive Serverのルートとして起動してください。
3. Live Serverのポート番号を確認してください。通常は `http://127.0.0.1:5500` です。ポート番号が異なる場合は、以降のURLを読み替えてください。
4. ブラウザのアドレス欄へ、確認したいURLを直接入力してください。
...
完成解答を確認する場合は、同じ手順で `exercise_sample\Ex4-4` フォルダをLive Serverのルートとして起動してください。
...
- 登録画面は、ブラウザのアドレス欄に `http://127.0.0.1:5500/employees/new` を直接入力して確認すること
- 更新モーダルは、ブラウザのアドレス欄に `http://127.0.0.1:5500/employees/1001` を直接入力し、社員詳細画面の更新ボタンから確認すること
- 社員一覧は、ブラウザのアドレス欄に `http://127.0.0.1:5500/employees` を直接入力して確認すること
