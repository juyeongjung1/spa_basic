...
app.get('/api/employees/:id', (req, res) => {
  db.get('SELECT * FROM employee WHERE id = ?', [req.params.id], (error, row) => {
    if (error) return res.status(500).json({ error: error.message });
    if (!row) return res.status(404).json({ error: '社員情報が見つかりません。' });
    res.json(row);
  });
});
...
app.post('/api/employees', (req, res) => {
  ...
  db.run(
    ...
    [employee.id, employee.password, employee.name, employee.salary, employee.location_name, employee.image_path],
    (error) => error ? res.status(500).json({ error: error.message }) : res.status(201).json(employee)
  );
});
...
app.put('/api/employees/:id', (req, res) => {
  ...
  db.run(
    ...
    (error) => error ? res.status(500).json({ error: error.message }) : res.json({ id: req.params.id, ...employee })
  );
});
