...
app._____(_____,(req,res)=>{
    ...
    db._____(sql,[id],err=>{if(err){res.status(500).json({error:'Database error'});return;}db._____('SELECT * FROM employee',[],(err,rows)=>{if(err){res.status(500).json({error:'Database error'});return;}_____});});
});
