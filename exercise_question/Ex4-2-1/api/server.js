...
app._____(_____,(req,res)=>{
    ...
    const id=_____;const sql=_____;
    db._____(sql,[id],(err,row)=>{if(err){res.status(500).json({error:'Database error'});return;}if(!row){res.status(404).json({error:'社員が見つかりません'});return;}_____});
});
