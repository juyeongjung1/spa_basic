...
app._____(_____,(req,res)=>{
    ...
    const id=_____;const {_____}=req.body;
    // 【手順3】UPDATE文とparamsを作成してください。
    const sql=_____;const params=_____;
    // 【手順4】db.runで更新し、成功後に最新一覧を返してください。
    db._____(sql,params,err=>{if(err){res.status(500).json({error:'Database error'});return;}db._____('SELECT * FROM employee',[],(err,rows)=>{if(err){res.status(500).json({error:'Database error'});return;}_____});});
});
