...
navigation.addEventListener('navigate',event=>{
    ...
    if(!event.canIntercept||url.origin!==window.location.origin){return;}
    event.intercept({handler:()=>showPage(url.pathname)});
});

function showPage(path){
    // 画面移動時に、前画面が作成したModalを残さないよう空にします。
    document.getElementById('modal-area').innerHTML='';
    if(path==='/'||path==='/index.html'){showHome();return;}
    if(path==='/employees'||path==='/employees/'){showEmployeeList();return;}
    if(path==='/employees/new'||path==='/employees/new/'){showEmployeeRegister();return;}
    let result=path.match(/^\/employees\/(\d+)$/);
    if(result){showEmployeeDetail(result[1]);return;}
